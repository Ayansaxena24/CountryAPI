  ...
  constructor(props: CountryInfoProps) {
    ...
    this.state = {
      weatherInfo: {
        ...
        "dt":1706007255,
        "sys":{
          "type": 0, // Add the missing property 'type'
          "id": 0, // Add the missing property 'id'
          "country":"IN",
          "sunrise":1705973369,
          "sunset":1706013444
        },
      ...
      countryData: this.props.location.state.countryDetails,
  ...
  handleWeatherData = async () => {
    ...
    try {
      const res = await fetch(
        `https://api.openweathermap.org/data/2.5/weather?lat=${countryData[0].latlng[0]}&lon=${countryData[0].latlng[1]}&appid=ba21f1fb2256fc495bceed24839bca72`
        );
        console.log("asdf")
        ...
        <div className="mt-4 p-4 border-2 rounded-md z-20 overflow-auto left-[50%] right-[50%] absolute w-[100vh] top-20 bg-opacity-70 bg-black ">
          ...
          >
            <Table aria-label="simple table">
              ...
              <TableBody>
                {(weatherInfo.coord.lon === 0 && weatherInfo.coord.lat ===0)  ? (
                  <TableRow>
                    <TableCell
                      colSpan={2}
                      align="center"
