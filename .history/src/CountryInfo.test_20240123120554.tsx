...
describe("CountryInfo", () => {
  ...
  it('should handle fetchWeatherData', async () => {
    ...
    global.fetch = jest.fn().mockResolvedValue({
      ...
      await act(async () => {
        wrapper.find('#capital').simulate('click'); // Replace 'yourButtonId' with the actual button ID or selector
        // await new Promise(resolve => setImmediate(resolve)); // Wait for state updates
        wrapper.update();
      });
  
      expect(global.fetch).toHaveBeenCalledWith(
        `https://api.openweathermap.org/data/2.5/weather?lat=20&lon=77&appid=ba21f1fb2256fc495bceed24839bca72`
      );
    });

    it('should show alert if handleWeatherData fails', async () => {
        const wrapper = shallow(<CountryInfo />) as any;
        global.fetch = jest.fn().mockRejectedValueOnce(new Error('fake error message'));
